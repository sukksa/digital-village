...
<script setup>
  ...
  const damidata = ref(Array.from({ length: 6 }, () => Math.floor(Math.random() * 140)))
  const shucaidata = ref(Array.from({ length: 6 }, () => Math.floor(Math.random() * 140)))
  const initChart = () => {
    if (chartRef.value) {
      ...
      var option = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            return params
              .map(item => {
                return `${item.seriesName}: ${item.value}家`
              })
              .join('<br/>')
          },
